...
local function processListingInfo(uid, gems, item, version, shiny, amount, boughtFrom, boughtStatus, class, failMessage, snipeNormal)
    ...
    local mention = (Library.Directory.Pets[item].huge or Library.Directory.Pets[item].titanic or Library.Directory.Pets[item]) and "<@" .. userid .. ">" or ""
        ...
        ['embeds'] = {
            {
                ...
                ['thumbnail'] = {
                    ["url"] = "https://cdn.discordapp.com/emojis/1179513460594319380.webp?size=48&name=thumbs_up&quality=lossless"
                },
                ['fields'] = {
                    {
                        ['name'] = "__Price:__",
    ...
    if type(message) == "table" then
        ...
        if listing then
            ...
            elseif item == "Booth Slot Voucher" and unitGems <= 25000 then
                coroutine.wrap(tryPurchase)(uid, gems, item, version, shiny, amount, username, class, playerid, buytimestamp, listTimestamp, snipeNormal)
                return
            elseif item == "Clan Voucher" and unitGems <= 100000 then
                coroutine.wrap(tryPurchase)(uid, gems, item, version, shiny, amount, username, class, playerid, buytimestamp, listTimestamp, snipeNormal)
                return
